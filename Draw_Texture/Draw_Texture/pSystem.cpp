...
Snow::Snow(BoundingBox_1 * boundingBox, int numParticles)
{
...
ParticleGun::ParticleGun(Xfile* xfile)
{
	_xfile          = xfile;
	_size            = 0.8f;
	_vbSize          = 2048;
...
void ParticleGun::resetParticle(Attribute* attribute)
{
	...
	//D3DXVECTOR3 xfilePos;

	//xfilePos=_xfile->GetPosistion(xfilePos)*100;
	//xfilePos=D3DXVECTOR3(-10.0f,10.0f,0.f);//写了一个定的值，但是为什么不能用getposistion

	D3DXVECTOR3 xfileDir;
	_xfile->getLook(&xfileDir);
	// change to camera position
	//attribute->_position = xfilePos;
	...
	// travels in the direction the camera is looking
	attribute->_velocity = xfileDir * 0.5f;
